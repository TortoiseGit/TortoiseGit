...
static int wcstristr(const wchar_t *a, const wchar_t *b)
{
	const wchar_t *tmpA, *tmpB, *end;
	...
	size_t lenTmpB = wcslen(tmpB);
	end = tmpA + wcslen(tmpA) - lenTmpB;
	while (tmpA <= end) {
		if (!wcsnicmp(tmpA, tmpB, lenTmpB))
			return 1;
		++tmpA;
	}

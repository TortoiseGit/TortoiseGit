...
int CGit::GetCurrentBranchFromFile(const CString &sProjectRoot, CString &sBranchOut, bool fallback)
{
	...
	else if (fallback)
	{
		...
		if (CGitHash::IsValidHash(unicodeHash, g_Git.GetCurrentRepoHashType()))
...
int CGit::GetHash(git_repository * repo, CGitHash &hash, const CString& friendname, bool skipFastCheck /* = false */)
{
	...
	if (!skipFastCheck && CGitHash::IsValidHash(friendname, GetRepository_oid_Type(repo)))
	{
		hash = CGitHash::FromHexStr(friendname, GetRepository_oid_Type(repo));
		return 0;
	}
...
int CGit::GetHash(CGitHash &hash, const CString& friendname)
{
	...
	// no need to parse a ref if it's already a 40-byte hash
	if (CGitHash::IsValidHash(friendname, hashType))
	{
		hash = CGitHash::FromHexStr(friendname, hashType);
		return 0;
	}
	...
	else
	{
		...
		hash = CGitHash::FromHexStrTry(gitLastErr.Trim(), hashType);
		if (ret == 0)
			gitLastErr.Empty();
...
int CGit::GetCommitDiffList(const CString &rev1, const CString &rev2, CTGitPathList &outputlist, bool ignoreSpaceAtEol, bool ignoreSpaceChange, bool ignoreAllSpace , bool ignoreBlankLines)
{
	...
	if (rev1 == GitRev::GetWorkingCopy(GetCurrentRepoHashType()) || rev2 == GitRev::GetWorkingCopy(GetCurrentRepoHashType()))
	{
		if (rev1 == GitRev::GetWorkingCopy(GetCurrentRepoHashType()))
...
int CGit::GetRemoteRefs(const CString& remote, REF_VECTOR& list, bool includeTags, bool includeBranches)
{
	...
	if (Run(
		cmd, [&](CStringA lineA) {
			CGitHash hash = CGitHash::FromHexStr(lineA.Left(hashLength), hashType);
			lineA = lineA.Mid(hashLength + static_cast<int>(wcslen(L"\t"))); // OID, tab
			if (lineA.IsEmpty())
				return;
...
int CGit::GetMapHashToFriendName(MAP_HASH_NAME &map)
{
	...
	const int ret = Run(L"git.exe show-ref -d", [&](const CStringA& lineA)
	{
		...
		CGitHash hash = CGitHash::FromHexStr(lineA, hashType);
...
bool CGit::IsFastForward(const CString &from, const CString &to, CGitHash * commonAncestor)
{
	...
	basehash = CGitHash::FromHexStrTry(base.Trim(), m_hashType);
...
{
	CString cmd;
	if (rev2 == GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()))
		cmd.Format(L"git.exe diff --stat%s -p --end-of-options %s --", bNoPrefix ? L" --no-prefix" : L"", static_cast<LPCWSTR>(rev1));
	else if (rev1 == GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()))
...
static int GetUnifiedDiffLibGit2(const CTGitPath& path, const CString& revOld, const CString& revNew, std::function<void(const git_buf*, void*)> statCallback, git_diff_line_cb callback, void* data, bool /* bMerge */, bool bNoPrefix)
{
	...
	if (revNew == GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()) || revOld == GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()))
	{
		CAutoTree t1;
		CAutoDiff diff2;

		if (revNew != GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()) && resolve_to_tree(repo, tree1, t1.GetPointer()))
			return -1;

		if (revOld != GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()) && resolve_to_tree(repo, tree2, t1.GetPointer()))
			return -1;
...
int CGit::ParseConflictHashesFromLsFile(const BYTE_VECTOR& out, CGitHash& baseHash, bool& baseIsFile, CGitHash& mineHash, bool& mineIsFile, CGitHash& remoteHash, bool& remoteIsFile)
{
	...
	while (pos < end)
	{
		...
		{
			baseHash = CGitHash::FromHexStrTry(hash, hashType);
			baseIsFile = mode != 160000;
		}
		else if (stage == 2)
		{
			mineHash = CGitHash::FromHexStrTry(hash, hashType);
			mineIsFile = mode != 160000;
		}
		else if (stage == 3)
		{
			remoteHash = CGitHash::FromHexStrTry(hash, hashType);

				...
				if (m_dwContextMenus & GITSLC_POPPREPAREDIFF && !(wcStatus & CTGitPath::LOGACTIONS_DELETED))
				{
					...
					if (!m_sMarkForDiffFilename.IsEmpty())
					{
						...
						else
						{
							...
							if (m_sMarkForDiffVersion != GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()) || PathIsRelative(m_sMarkForDiffFilename))
			...
			case IDGITLC_COMPARETWOFILES:
				{
					...
					if (pos)
					{
						...
						CString sCmd;
						if (m_CurrentVersion.IsEmpty())
							sCmd.Format(L"/command:diff /path:\"%s\" /startrev:%s /path2:\"%s\" /endrev:%s /hwnd:%p", firstfilepath->GetWinPath(), firstfilepath->Exists() ? GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()) : GitRev::GetHead(), secondfilepath->GetWinPath(), secondfilepath->Exists() ? GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()) : GitRev::GetHead(), reinterpret_cast<void*>(m_hWnd));
						else
			...
			case IDGITLC_GNUDIFF1:
				{
					...
					while (pos)
					{
						...
						if (m_CurrentVersion.IsEmpty())
						{
							...
							if (m_amend)
								fromwhere = L"~1";
							if (g_Git.GetUnifiedDiff(*selectedFilepath, GitRev::GetHeadString() + fromwhere, GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()), tempfile, false, false, diffContext, false))
							{
						...
						else
						{
							if ((selectedFilepath->m_ParentNo & (PARENT_MASK | MERGE_MASK)) == 0)
							{
								...
								{
									if (g_Git.GetUnifiedDiff(*selectedFilepath, GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()), m_CurrentVersion.ToString(), tempfile, false, false, diffContext, false))
									{
										::MessageBox(m_hWnd, g_Git.GetGitLastErr(L"Could not get unified diff.", CGit::GIT_CMD_DIFF), L"TortoiseGit", MB_OK);
										break;
									}
...
void CGitStatusListCtrl::StartDiffWC(int fileindex, bool parent)
{
	...
	CGitDiff::Diff(GetParentHWND(), &file1, &file1, GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()), version, false, false, 0, !!(GetAsyncKeyState(VK_SHIFT) & 0x8000));
}
...
void CGitStatusListCtrl::StartDiff(int fileindex)
{
	...
	if (m_CurrentVersion.IsEmpty())
	{
		...
		if( g_Git.IsInitRepos())
			...
			GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()), true, 0, !!(GetAsyncKeyState(VK_SHIFT) & 0x8000));
			...
			CGitDiff::Diff(GetParentHWND(), &file1,&file2,
					GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()),
...
void CGitStatusListCtrl::UpdateDiffWithFileFromReg()
{
	...
	if (CString diffLaterFile = CRegString(L"Software\\TortoiseGit\\DiffLater", L""); !diffLaterFile.IsEmpty() && lastDiffLaterFile != diffLaterFile)
	{
		...
		m_sMarkForDiffVersion = GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType());
	}
}

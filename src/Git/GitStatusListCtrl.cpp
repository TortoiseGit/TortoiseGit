			...
			case IDGITLC_COMPARETWOFILES:
				{
					...
					if (pos)
					{
						...
						CString sCmd;
						if (m_CurrentVersion.IsEmpty())
							sCmd.Format(L"/command:diff /path:\"%s\" /startrev:%s /path2:\"%s\" /endrev:%s /hwnd:%p", firstfilepath->GetWinPath(), firstfilepath->Exists() ? GitRev::GetWorkingCopy() : GitRev::GetHead(), secondfilepath->GetWinPath(), secondfilepath->Exists() ? GitRev::GetWorkingCopy() : GitRev::GetHead(), reinterpret_cast<void*>(m_hWnd));
						else
...
void CGitStatusListCtrl::StartDiffWC(int fileindex, bool parent)
{
	...
	CGitDiff::Diff(GetParentHWND(), &file1, &file1, GitRev::GetWorkingCopy(), version, false, false, 0, !!(GetAsyncKeyState(VK_SHIFT) & 0x8000));
}
...
void CGitStatusListCtrl::StartDiff(int fileindex)
{
	...
	if (m_CurrentVersion.IsEmpty())
	{
		...
		if( g_Git.IsInitRepos())
			...
			GitRev::GetWorkingCopy(), true, 0, !!(GetAsyncKeyState(VK_SHIFT) & 0x8000));

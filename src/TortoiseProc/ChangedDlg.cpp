...
void CChangedDlg::OnBnClickedButtonUnifieddiff()
{
	...
	CString sCmd;
	sCmd.Format(L"/command:showcompare /unified /path:\"%s\" /revision1:HEAD /revision2:%s", static_cast<LPCWSTR>(g_Git.CombinePath(commonDirectory)), static_cast<LPCWSTR>(GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType())));

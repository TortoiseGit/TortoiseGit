...
int CGitDiff::SubmoduleDiffNull(HWND hWnd, const CTGitPath* pPath, const CGitHash& hash)
{
	...
	if(start>0)
	{
		...
		if(start>0)
			newhash = CGitHash::FromHexStrTry(output.Mid(start + 1, CGitHash::HashLength(g_Git.GetCurrentRepoHashType()) * 2), g_Git.GetCurrentRepoHashType());
...
{
	CGitHash rev1Hash;
	if (rev1 != GitRev::GetWorkingCopy(g_Git.GetCurrentRepoHashType()))
	{
...
int CGitDiff::SubmoduleDiff(HWND hWnd, const CTGitPath* pPath, const CTGitPath* /*pPath2*/, const CGitHash& rev1, const CGitHash& rev2, bool /*blame*/, bool /*unified*/)
{
	...
	if (rev2.IsEmpty() || rev1.IsEmpty())
	{
		...
		oldhash = CGitHash::FromHexStrTry(output.Mid(oldstart + static_cast<int>(wcslen(L"-Subproject commit")) + 1, CGitHash::HashLength(g_Git.GetCurrentRepoHashType()) * 2), g_Git.GetCurrentRepoHashType());
		start = 0;
		...
		newhash = CGitHash::FromHexStrTry(output.Mid(newstart + static_cast<int>(wcslen(L"+Subproject commit")) + 1, CGitHash::HashLength(g_Git.GetCurrentRepoHashType()) * 2), g_Git.GetCurrentRepoHashType());
	...
	else
	{
		...
		CString temp;
		CGit::StringAppend(temp, &bytes[strlen(":000000 000000 ")], CP_UTF8, hashLength);
		oldhash = CGitHash::FromHexStrTry(temp, g_Git.GetCurrentRepoHashType());
		temp.Empty();
		CGit::StringAppend(temp, &bytes[strlen(":000000 000000 ") + hashLength + 1], CP_UTF8, hashLength);
		newhash = CGitHash::FromHexStrTry(temp, g_Git.GetCurrentRepoHashType());
	}

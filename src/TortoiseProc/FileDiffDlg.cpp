...
void CFileDiffDlg::SetDiff(const CTGitPath* path, const CString &baseRev1, const CString& hash2)
{
	...
	if (baseRev1 == GitRev::GetWorkingCopy())
	{
	...
	if (hash2 == GitRev::GetWorkingCopy())
	{
...
void CFileDiffDlg::DoDiff(int selIndex, bool blame)
{
	...
	{
		CGitDiff::DiffNull(GetSafeHwnd(), fd2, GitRev::GetWorkingCopy(), true, 0, !!(GetAsyncKeyState(VK_SHIFT) & 0x8000));
		return;
	}

...
//			W_Remotes.DoModal();
	...
	case eCmd_DiffWC:
		{
			CString sCmd;
			sCmd.Format(L"/command:showcompare /path:\"%s\" /revision1:%s /revision2:%s", static_cast<LPCWSTR>(g_Git.m_CurrentDir), static_cast<LPCWSTR>(selectedLeafs[0]->GetRefName()), GitRev::GetWorkingCopyRef(g_Git.GetCurrentRepoHashType()));

...
static void GetSubmodulePathList(STRING_VECTOR &list, STRING_VECTOR &prefixList)
{
	...
	{
		MessageBox(NULL, g_Git.GetGitLastErr(_T("Could not open repository.")), _T("TortoiseGit"), MB_ICONERROR);
		return;
	}
	...
	if (git_submodule_foreach(repo, SubmoduleCallback, &listParams))
	{
		...
		MessageBox(NULL, g_Git.GetGitLastErr(_T("Could not get submodule list.")), _T("TortoiseGit"), MB_ICONERROR);
		return;
	}

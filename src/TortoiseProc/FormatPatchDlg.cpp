...
void CFormatPatchDlg::OnBnClickedButtonUnifieddiff()
{
	UpdateData(TRUE);
	m_regNoPrefix = m_bNoPrefix;
	CAppUtils::StartShowUnifiedDiff(m_hWnd, CTGitPath(), L"HEAD", CTGitPath(), GitRev::GetWorkingCopyRef(g_Git.GetCurrentRepoHashType()), !!(GetAsyncKeyState(VK_SHIFT) & 0x8000), false, false, false, false, !!m_bNoPrefix);
}

...
Overall, Python 3 (can be installed from the Microsoft Store) is required.
Scripts are included, but executables and DocBook XSL have to be installed separately.
For building the PDF manual, you will also need to have a recent Java Runtime Environment.

    tools\fop\		- the Apache fop processor (for PDF generation, https://xmlgraphics.apache.org/fop/)
    tools\xsl\		- the DocBook XSL 1.78.1 (https://sourceforge.net/projects/docbook/files/docbook-xsl/1.78.1/)
    tools\		  	- xslt processor (xsltproc)
...
to build other docs too. This will, however, require tweaking the configuration for the build scripts (cf. doc.build.user.tmpl).

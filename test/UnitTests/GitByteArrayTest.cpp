...
TEST(CGitByteArray, FindData)
{
	BYTE inputByteArray[] = { "1234\0""5789\0" };
	CGitByteArray byteArray;

	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"\0", 1));
	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"3", 1));
	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"\0", 1, 15));

	EXPECT_EQ(0, byteArray.append(inputByteArray, sizeof(inputByteArray)));
	EXPECT_EQ(sizeof(inputByteArray), byteArray.size());

	EXPECT_EQ(0, byteArray.findData((const BYTE*)"\0", 0));
	EXPECT_EQ(0, byteArray.findData((const BYTE*)"\0", 0, 5));

	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"1234567890123", 13));
	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"1234567890123", 13, 5));

	EXPECT_EQ(4, byteArray.findData((const BYTE*)"\0", 1));
	EXPECT_EQ(4, byteArray.findData((const BYTE*)"\0", 1, 4));
	EXPECT_EQ(9, byteArray.findData((const BYTE*)"\0", 1, 5));
	// out of bounds access dies

	EXPECT_EQ(0, byteArray.findData((const BYTE*)"1", 1));
	EXPECT_EQ(1, byteArray.findData((const BYTE*)"2", 1));

	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"F", 1));

	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"132", 3));
	EXPECT_EQ(1, byteArray.findData((const BYTE*)"23", 2));
	EXPECT_EQ(3, byteArray.findData((const BYTE*)"4\0""5", 3));

	EXPECT_EQ(0, byteArray.append((BYTE*)"1", 1));
	EXPECT_EQ(0, byteArray.findData((const BYTE*)"1", 1));
	EXPECT_EQ(11, byteArray.findData((const BYTE*)"1", 1, 2));

	EXPECT_EQ(4, byteArray.findData((const BYTE*)"\0", 1));
	EXPECT_EQ(4, byteArray.findData((const BYTE*)"\0", 1, 4));
	EXPECT_EQ(9, byteArray.findData((const BYTE*)"\0", 1, 5));
	EXPECT_EQ(9, byteArray.findData((const BYTE*)"\0", 1, 9));
	EXPECT_EQ(10, byteArray.findData((const BYTE*)"\0", 1, 10));
	EXPECT_EQ(-1, byteArray.findData((const BYTE*)"\0", 1, 11));
}

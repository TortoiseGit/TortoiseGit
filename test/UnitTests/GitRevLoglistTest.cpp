...
static void SafeFetchFullInfo(CGit* cGit)
{
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"7c3cbfe13a929d2291a574dca45e4fd2d2ac1aa6", GIT_HASH_TYPE::GIT_HASH_SHA1);
	...
	rev.Clear();
	EXPECT_EQ(0, rev.GetFiles(nullptr).GetCount());
	rev.m_CommitHash = CGitHash::FromHexStr(L"dead91b4aedeaddeaddead2a56d3c473c705dead", GIT_HASH_TYPE::GIT_HASH_SHA1); // non-existent commit
	EXPECT_EQ(-1, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsSimpleListReady);
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"35c91b4ae2f77f4f21a7aba56d3c473c705d89e6", GIT_HASH_TYPE::GIT_HASH_SHA1);
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, rev.GetAction(nullptr));
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"8d1ebbcc7eeb63af10ff8bcf7712afb9fcc90b8a", GIT_HASH_TYPE::GIT_HASH_SHA1); // merge commit
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	...
	EXPECT_FALSE(list[0].IsDirectory());
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"18da7c332dcad0f37f9977d9176dce0b0c66f3eb", GIT_HASH_TYPE::GIT_HASH_SHA1); // stash commit
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, rev.GetAction(nullptr));
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"844309789a13614b52d5e7cbfe6350dd73d1dc72", GIT_HASH_TYPE::GIT_HASH_SHA1); // root commit
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(CTGitPath::LOGACTIONS_ADDED, rev.GetAction(nullptr));
	ASSERT_EQ(1, rev.GetFiles(nullptr).GetCount());
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"4c5c93d2a0b368bc4570d5ec02ab03b9c4334d44", GIT_HASH_TYPE::GIT_HASH_SHA1); // signed commit with renamed file last
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"31ff87c86e9f6d3853e438cb151043f30f09029a", GIT_HASH_TYPE::GIT_HASH_SHA1); // commit with renamed file first
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(CTGitPath::LOGACTIONS_ADDED | CTGitPath::LOGACTIONS_MODIFIED | CTGitPath::LOGACTIONS_REPLACED | CTGitPath::LOGACTIONS_DELETED, rev.GetAction(nullptr));
	ASSERT_EQ(4, rev.GetFiles(nullptr).GetCount());
	list = rev.GetFiles(nullptr).m_files;
...
static void SafeFetchFullInfo_Submodule(CGit* cGit, config testConfig)
{
	...
	rev.m_CommitHash = CGitHash::FromHexStr(L"900539cd24776a94d1b642358ccfdb9d897c8254", GIT_HASH_TYPE::GIT_HASH_SHA1); // added submodule
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	EXPECT_EQ(CTGitPath::LOGACTIONS_ADDED, rev.GetAction(nullptr));
	ASSERT_EQ(1, rev.GetFiles(nullptr).GetCount());
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"c8d17f57c7b511aff4aa2fbfae158902281cad8e", GIT_HASH_TYPE::GIT_HASH_SHA1); // modified submodule
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, rev.GetAction(nullptr));
	ASSERT_EQ(1, rev.GetFiles(nullptr).GetCount());
	...
	EXPECT_TRUE(list[0].IsDirectory());
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"4ed8d1f9ce9aedc6ad044d9051cb584a8bc294ac", GIT_HASH_TYPE::GIT_HASH_SHA1); // deleted submodule
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	...
	EXPECT_TRUE(list[0].IsDirectory());
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"2e63f1a55bc3dce074897200b226009f575fbcae", GIT_HASH_TYPE::GIT_HASH_SHA1); // submodule to file
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, rev.GetAction(nullptr));
	ASSERT_EQ(1, rev.GetFiles(nullptr).GetCount());
	...
	EXPECT_FALSE(list[0].IsDirectory());
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"07ac6e5916c03747f7485195deb7ec9100d1c2ef", GIT_HASH_TYPE::GIT_HASH_SHA1); // file to submodule
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, rev.GetAction(nullptr));
	ASSERT_EQ(1, rev.GetFiles(nullptr).GetCount());
	...
	EXPECT_FALSE(list[0].IsDirectory());
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"2d2017245cf3d016c64e5ad4eb6b0f1bccd1cf7f", GIT_HASH_TYPE::GIT_HASH_SHA1); // merge use third
	EXPECT_EQ(0, rev.SafeFetchFullInfo(cGit));
	EXPECT_EQ(FALSE, rev.m_IsDiffFiles);
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, rev.GetAction(nullptr));
	ASSERT_EQ(2, rev.GetFiles(nullptr).GetCount());
...
static void SafeGetSimpleList(CGit* cGit)
{
	...
	rev.Clear();
	rev.m_CommitHash = CGitHash::FromHexStr(L"4c5c93d2a0b368bc4570d5ec02ab03b9c4334d44", GIT_HASH_TYPE::GIT_HASH_SHA1);
	EXPECT_EQ(0, rev.SafeGetSimpleList(cGit));
